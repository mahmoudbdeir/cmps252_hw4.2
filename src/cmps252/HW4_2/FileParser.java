	...
	public static List<Customer> getCustomers(String file) throws FileNotFoundException {
		...
		try (Scanner scanner = new Scanner(new File(file));) {
			...
			while (scanner.hasNextLine()) {
				...
				Customer s = new Customer(rmv(fields[0]), rmv(fields[1]), rmv(fields[2]), rmv(fields[3]),
						rmv(fields[4]), rmv(fields[5]), rmv(fields[6]), rmv(fields[7]), rmv(fields[8]), rmv(fields[9]),
